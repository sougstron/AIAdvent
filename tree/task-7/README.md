...
```sh
cargo build --release --manifest-path tree/task-7/Cargo.toml
# бинарник: tree/task-7/target/release/ask
# target/ в gitignore — артефакт сборки в снапшот не коммитится

./tree/task-7/target/release/ask                         # чат-TUI
./tree/task-7/target/release/ask "вопрос"                # one-shot
./tree/task-7/target/release/ask --models               # каталог: live / paid / no key
./tree/task-7/target/release/ask --login glm             # подключить ключ (живая проверка)
./tree/task-7/target/release/ask --keys                  # таблица ключей и источников
./tree/task-7/target/release/ask --verify-login          # живо перепроверить ключи
./tree/task-7/target/release/ask --verify                # живой self-test рычагов
./tree/task-7/target/release/ask --sessions              # список сессий
./tree/task-7/target/release/ask --continue              # последняя сессия
./tree/task-7/target/release/ask --resume ID             # сессия по id
```
